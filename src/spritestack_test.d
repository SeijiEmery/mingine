...
struct VoxelAsset {
    ...
    static struct Voxel           { ubyte x, y, z, i; }
    ...
    static struct VoxelColor      { ubyte r, g, b, a; }
    ...
    bool tryLoadAsset(string name, string filePath) {
        ...
        try {
            ...
            colorPalette = VoxelColorPalette((cast(VoxelColor[])DEFAULT_PALETTE).dup);
            ...
            foreach (chunk; chunks) {
                switch (chunk.type) {
                    case "RGBA": {
                        ...
                        this.colorPalette = readColorPalette(chunk); 
                    } break;
                    case "MATL": break;
                    case "rOBJ": break;
            ...
            Voxel[] invalidVoxels;
            foreach (model; voxelModels) {
                foreach (voxel; model.voxels) {
                    if (voxel.i >= colorPalette.palette.length) {
                        invalidVoxels ~= voxel;
                    } 
                }
            }
